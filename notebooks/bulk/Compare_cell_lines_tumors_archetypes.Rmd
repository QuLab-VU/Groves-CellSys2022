...
#### Variables relevant to paths on your computer
May run without changing; default is a directory on the Desktop.
```{r Output}
OUTPUT_DIR <- file.path("~/Desktop/SCLC_archetypes_output")
OVERWRITE <- FALSE

if(!dir.exists(OUTPUT_DIR)) dir.create(OUTPUT_DIR)
```

#### Load data and filter genes by mean difference
```{r }
...
SAmapping_clines56_savepath <- file.path(OUTPUT_DIR,"SAmapping_clines56.Rdata")
if(!file.exists(SAmapping_clines56_savepath) | OVERWRITE) 
    save.image(SAmapping_clines56_savepath)
