...
genes_df = pd.read_csv('/Users/smgroves/Documents/MATLAB/particode/out/Bulk_final_genes__continuous_significant.csv', header = 0)
genes_df = genes_df.loc[genes_df['Significant after Benjamini-Hochberg correction?'] == 1]
...
arc_sig_df = mb.ar.generate_arc_sig_df(genes_df, features_name = 'Feature Name', logfc_name = 'Mean Difference', 
                    p_adj_name = 'P value (Mann-Whitney)', subtype_name = 'archetype #', subtype_dict = subtype_dict)
...
sig_matrix_pure, cond, n_genes = mb.ar.generate_signature(archetypes, arc_sig_df, norm = None,
                                           range_min=20, range_max=100, range_step=1, sort_by_ascending=False,
                                           print_c = False)


sig_matrix_pure.to_csv('sig_matrix_matlab.csv')
