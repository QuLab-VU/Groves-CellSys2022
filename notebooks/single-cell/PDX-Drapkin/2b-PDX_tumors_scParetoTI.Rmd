---
title: "Single Cell SCLC PDX Tumor Archetypes"
output:
  html_document:
    df_print: paged
---
...
```{r message = FALSE}
file = "../../out/PDX-tumors/X_magic_for_R.h5ad"
Convert(file, dest = "h5seurat", overwrite = TRUE)
data <- LoadH5Seurat("../../out/PDX-tumors/X_magic_for_R.h5seurat")
...
save(data, file="../../out/PDX-tumors/ParetoTI/X_magic_for_ParetoTI.Robj")

write.csv(data[["RNA"]]@scale.data, '../../out/PDX-tumors/ParetoTI/integrated-corrected-data.csv')
write.csv(data[['pca']]@cell.embeddings, '../../out/PDX-tumors/ParetoTI/pca_embedding.csv')
write.csv(data[['pca']]@feature.loadings, '../../out/PDX-tumors/ParetoTI/pca_feature_loadings.csv')

data <- ProjectDim(data, reduction = 'pca')
write.csv(data[['pca']]@feature.loadings.projected, '../../out/PDX-tumors/ParetoTI/pca_feature_loadings_projected.csv')
...
x_pca <- read.csv('../../out/PDX-tumors/ParetoTI/pca_embedding.csv',header = TRUE,row.names = 1)
x_pca <- t(x_pca)

x_pca <- x_pca[1:8,] #keep only top 8 PCs
loadings<- read.csv('../../out/PDX-tumors/ParetoTI/pca_feature_loadings.csv', header = TRUE, row.names = 1)
...
pdf(sprintf('../../figures/ParetoTI/PDX-tumors/%s_t-ratio_test.pdf', i))
...
save(arc, file="../../out/PDX-tumors/ParetoTI/arc.Robj")
save(arc_rob, file="../../out/PDX-tumors/ParetoTI/arc_rob.Robj")
save(arc_ave, file="../../out/PDX-tumors/ParetoTI/arc_ave.Robj")
write.csv(arc$XC, file="../../out/PDX-tumors/ParetoTI/arc_positions_pca.csv")
write.csv(arc_ave$XC, file="../../out/PDX-tumors/ParetoTI/arc_ave_positions_pca.csv")

reconstruct_from_pca <- function(x, loadings, arc_XC, nComp = 8){
...
write.csv(t(arc_genespace), '../../out/PDX-tumors/ParetoTI/arc_gene-space.csv')
##################################
# 
...
htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/robust_archetypes.html")
# 
...
# htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/NEUROD2.HTML")
# 
# p_pca = plot_arc(arc_data = arc_ave, data = x_pca, 
#                  which_dimensions = 1:3, line_size = 1.5,
...
# htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/YAP1.html")
# 
# # p_pca = plot_arc(arc_data = arc_ave, data = x_pca, 
#                  which_dimensions = 1:3, line_size = 1.5,
...
# htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/POU2F3.html")
...
htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/ASCL1.html")
...
htmlwidgets::saveWidget(p_pca, "../../figures/ParetoTI/PDX-tumors/MYC.html")
...
```{r}
library(matrixStats)
load(file="../../out/PDX-tumors/ParetoTI/arc.Robj")
# load( file="../int/single-cell/arc_rob.Robj")
# load( file="../int/single-cell/arc_ave.Robj")
write.csv(t(arc$S), '../../out/PDX-tumors/ParetoTI/S.csv')
...
save(data_attr, file="../../out/PDX-tumors/ParetoTI/data_attr.Robj")
...
write.csv(enriched_genes, '../../out/PDX-tumors/ParetoTI/enriched-genes.csv')

enriched_sets = find_decreasing_wilcox(data_attr$data, data_attr$arc_col,
                                features = data_attr$colData_col,
                                bin_prop = 0.05, method = "BioQC")
write.csv(enriched_sets, '../../out/PDX-tumors/ParetoTI/enriched_sets.csv')
